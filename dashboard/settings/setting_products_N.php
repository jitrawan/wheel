...
if(isset($_POST['save_edit_item'])){
  if(addslashes($_POST['edit_ProductID']) != NULL){
    ...
    $Strsql .= " ,p.PriceBuy=".addslashes($_POST['edit_PriceBuy'])." , p.Warranty= '".addslashes($_POST['edit_Warranty'])."' ";
 ...
 <div class="panel panel-default">
  ...
  if(isset($_POST['search_product'])){
   if(addslashes($_POST['search_type']) == 1){
    if($_POST['search_diameterWheel'] != ""){
      ...
      $getproduct = $getdata->my_sql_selectJoin("p.*,r.*,w.* ","product_N","productDetailWheel w on p.ProductID = w.ProductID left join productDetailRubber r on p.ProductID = r.ProductID "," Where p.TypeID = '1' ".$str_sql);
    }else{
      ...
      $getproduct = $getdata->my_sql_selectJoin("p.*,r.*,w.*,p.ProductID as productMain ","product_N","productDetailWheel w on p.ProductID = w.ProductID left join productDetailRubber r on p.ProductID = r.ProductID ","Where p.TypeID = '2' ".$str_sql);
    }
  }else{
     $getproduct = $getdata->my_sql_selectJoin("p.*,r.*,w.*,p.ProductID as productMain ","product_N","productDetailWheel w on p.ProductID = w.ProductID left join productDetailRubber r on p.ProductID = r.ProductID "," where p.ProductStatus in ('1','2')  ORDER BY  p.ProductID ");
  }
  ...
  if(mysql_num_rows($getproduct) > 0){
    ...
    <tr style="color:#FFF;">
      ...
      <th width="15%" bgcolor="#5fb760">สินค้า</th>
      <th width="10%" bgcolor="#5fb760">Hand</th>
      <th width="10%" bgcolor="#5fb760">คงเหลือ</th>
    ...
    ?>
    <tr id="<?php echo @$showproduct->productMain;?>">
      ...
      <td align="center" valign="middle">
  	  ...
  	  if($showproduct->ProductStatus == '1'){
  		  echo '<button type="button" class="btn btn-success btn-xs" id="btn-'.@$showproduct->productMain.'" onClick="javascript:changeproductsStatus(\''.@$showproduct->productMain.'\',\''.$_SESSION['lang'].'\');"><i class="fa fa-unlock-alt" id="icon-'.@$showproduct->productMain.'"></i> <span id="text-'.@$showproduct->productMain.'">'.LA_BTN_ON.'</span></button>';
  	  }else{
  		  echo '<button type="button" class="btn btn-danger btn-xs" id="btn-'.@$showproduct->productMain.'" onClick="javascript:changeproductsStatus(\''.@$showproduct->productMain.'\',\''.$_SESSION['lang'].'\');"><i class="fa fa-lock" id="icon-'.@$showproduct->productMain.'"></i> <span id="text-'.@$showproduct->productMain.'">'.LA_BTN_OFF.'</span></button>';
  	  }
  	  ?><a data-toggle="modal" data-target="#edit_item" data-whatever="<?php echo @$showproduct->productMain;?>" class="btn btn-info btn-xs"><i class="fa fa-edit"></i> <?php echo @LA_BTN_EDIT;?></a><a onClick="javascript:deleteProduct('<?php echo @$showproduct->productMain;?>');" class="btn btn-danger btn-xs"><i class="fa fa-times"></i> <?php echo @LA_BTN_DELETE;?></a></td>
    </tr>
  <? }?>
  </tbody>
</table>
